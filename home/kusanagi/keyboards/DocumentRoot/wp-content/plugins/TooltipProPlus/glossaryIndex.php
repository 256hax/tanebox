    ...
    public static function outputGlossaryIndexPage($shortcodeAtts) {
        ...
        if ($glossary_index) {
            ...
            foreach ($glossary_index as $glossaryItem) {
                ...
                $titleAttr = (get_option('cmtt_showTitleAttribute') == 1) ? ' title="' . $titleAttrPrefix . ' ' . esc_attr($glossaryItem->post_title) . '" ' : '';
                ...
                $preItemTitleContent .= '<' . $tag . ' class="' . $glossary_list_class . ' ' . $additionalClass . '" ' . $titleAttr . ' ' . $href . ' ' . $windowTarget . ' ';
...
//			add_filter( 'the_content', array( __CLASS__, '_preContent' ), PHP_INT_MAX );
    ...
    public static function outputScripts() {
        ...
        $displayAnimation = get_option('cmtt_tooltipDisplayanimation');
        $hideAnimation = get_option('cmtt_tooltipHideanimation');
...
//            $scriptsConfig['styles']['jqueryui-css'] = array(
...
//                );
        ...
        $fontName = get_option('cmtt_tooltipFontStyle', 'default (disables Google Fonts)');
